...
int main(int argc, char **argv)
{
	...
	for (k = 0; k < 3; k++)
	{
		...
		/* Make binary image and compute area, xbar, ybar*/
		A = 0;
		xbar = 0;
		ybar = 0;
		for (i = 0; i < ROWS; i++) {
			for (j = 0; j < COLUMNS; j++) {
				if (image[i][j] < threshold[k]){
					bimage[i][j] = 255; // A value of 0 is object (black)
					A++;
					xbar += j;
					ybar += i;
				}
				else {
					bimage[i][j] = 0; // A value of 255 is background (white)
				}
			}
		}
		xbar = xbar / A;
		...
		/* Place Center Patch*/
		for (i = ybar - 2; i <= ybar + 2; i++) {
			for (j = xbar - 2; j <= xbar + 2; j++) {
				bimage[i][j] = 128;
			}
		}
