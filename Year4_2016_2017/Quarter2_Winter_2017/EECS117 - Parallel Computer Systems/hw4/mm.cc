...
#include <time.h>
#include <assert.h>
#include <x86intrin.h> // Add x86 intrinsics
...
typedef float dtype; //changed data type to float to accommodate SIMD
			...
			for (int k0 = 0; k0 < K; k0 += blocksize) {
				...
				for (int i1 = i0; i1 < imax; i1++) {
					for (int j1 = j0; j1 < jmax; j1+=4) {
						...
						//t2 = C[i1 * M + (j1 + 2)];
						//t3 = C[i1 * M + (j1 + 3)];
						cvec = _mm_load_ps(C + (i1 * M + j1));
						for (int k1 = k0; k1 < kmax; k1++) {
							avec = _mm_set_ps(A[i1 * K + k1], A[i1 * K + k1], A[i1 * K + k1], A[i1 * K + k1]);
							bvec = _mm_load_ps(B + (k1 * M + j1));
							//t0 += A[i1 * K + k1] * B[k1 * M + j1];
							//t1 += A[i1 * K + k1] * B[k1 * M + (j1 + 1)];
							//t2 += A[i1 * K + k1] * B[k1 * M + (j1 + 2)];
							//t3 += A[i1 * K + k1] * B[k1 * M + (j1 + 3)];
							tvec = _mm_mul_ps(avec, bvec);
							cvec = _mm_add_ps(cvec, tvec);
						}
						//C[i1 * M + j1] = t0;
						//C[i1 * M + (j1 + 1)] = t1;
						//C[i1 * M + (j1 + 2)] = t2;
						//C[i1 * M + (j1 + 3)] = t3;
						_mm_store_ps(C + (i1 * M + j1), cvec);
					}
				}
			}
		}
